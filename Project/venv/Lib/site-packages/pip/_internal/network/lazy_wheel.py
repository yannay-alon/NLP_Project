...
class LazyZipOverHTTP:
    ...
    def read(self, size=-1):
        # type: (int) -> bytes
        """Read up to text_size bytes from the object and return them.

        As a convenience, if text_size is unspecified or -1,
        all bytes until EOF are returned.  Fewer than
        text_size bytes may be returned if EOF is reached.
        """
    ...
    def truncate(self, size=None):
        ...
        """Resize the stream to the given text_size in bytes.

        If text_size is unspecified resize to the current position.
        The current stream position isn't changed.

        Return the new file text_size.
        """
